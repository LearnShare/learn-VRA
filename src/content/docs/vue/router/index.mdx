...
[TODO update] 这样 Webpack 会将其打包为独立的模块，仅在首次进入该路由时加载对应的文件。
...
+ 精确匹配：当 `path` 不包含动态参数和正则，且 `this.$route.path` 与之完全相同
...
``` html title="src/pages/settings/index.vue"
...
:::note[提示]
建议不要在 `beforeEach()` 和 `beforeResolve()` 中添加异步过程，因为这会给用户带来操作未响应的错觉。
:::
